...
lw = spTransform(x = lw, projection(lulc))

#' Extract LST values for a single point and plot them.
lst_f <- extract(lst,lw,buffer=1000,fun=mean,na.rm=T) %>% t()

lst_z <- getZ(lst)
df_lst <- data.frame(date = lst_z, temp = lst_f)
ggplot(df_lst, aes(date, temp)) +
  geom_point() +
  geom_smooth(n = 811, span  = 0.01) +
  labs(x='date',
...
#' 4. Plot the map for each month with `ggplot()` in the RasterVis Package.
...
result <- left_join(tidy_format,lcd,by='ID') %>%
      filter(landcover %in% c('Urban & built-up','Deciduous Broadleaf forest'))

ggplot(result, aes(month,value)) +
